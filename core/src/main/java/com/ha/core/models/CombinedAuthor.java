...
public class CombinedAuthor extends WCMUse {
    ...
    public void activate() throws Exception {
        ...
        if (!shipId.equalsIgnoreCase("")) {
            if (!session.nodeExists("/etc/combined/hal/ships/" + shipId)) {
                Node shipNode = JcrUtils.getOrCreateByPath("/etc/combined/hal/ships/", "nt:unstructured",  session);
            ...
            Node shipDescriptionNode = session.getNode("/etc/combined/hal/ships/" + shipId);
